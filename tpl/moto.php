...
<p class="session"><?php if(isset($_SESSION['res'])) {
	...
	<article class="article_desc">
		<p class="img_article"><a href="?option=view&amp;cat=<?=$row[1]?>&amp;id_news=<?=$row[0]?>"><img src="<?=$row[5]?>" width="184" alt="News"></a></p>
		<h3><a href="?option=view&amp;cat=<?=$row[1]?>&amp;id_news=<?=$row[0]?>"><?=$row[2]?></a></h3>
		<p class="time_article"><?=$row[4]?></p>
		<?=$row[3]?>
